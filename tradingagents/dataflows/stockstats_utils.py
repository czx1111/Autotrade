...
def _load_ohlcv_ashare(code: str, symbol: str, curr_date: str) -> pd.DataFrame:
    """A 股 OHLCV：走国产多源日 K（东财→腾讯→pytdx→新浪，带故障转移）。

    Yahoo 不收录无后缀的 A 股代码，且大陆网络通常无法访问 Yahoo；
    A 股代码统一改走 quote_sources.get_kline 的国内多源链路。缓存
    语义与 yfinance 路径一致（同窗一文件、当日 TTL 刷新）。
    """
    from .quote_sources import get_kline
    ...
    data_file = os.path.join(
        ...
        f"{safe_ticker_component(code)}-AKShare-data-{start_str}-{end_str}.csv",
    )
    ...
    if data is None:
        # 1825 天 ≈ 5 年：东财按自然日取窗口，腾讯按 bar 数取（≈7 年），
        # 均足够覆盖 200 SMA 等长周期指标。
        kline = get_kline(code, days=1825)
        if kline is None or kline.empty:
            raise NoMarketDataError(
                symbol, code,
                "all domestic kline vendors (EM/Tencent/pytdx/Sina) returned no rows",
            )
        fetched = kline.rename(columns={
...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    A 股代码（600519 / sh600519 / 600519.SH）走国内多源日 K；其余符号
    走 Yahoo Finance。
    """
    from .ashare_symbol_utils import normalize_ashare_symbol

    ashare_code = normalize_ashare_symbol(symbol)
    if ashare_code:
        return _load_ohlcv_ashare(ashare_code, symbol, curr_date)
