...
class ConditionalLogic:
    ...
    def should_continue_hotmoney(self, state: AgentState):
        """Determine if hot-money flow analysis should continue."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
            return "tools_hotmoney"
        return "Msg Clear Hotmoney"

    def should_continue_unlock(self, state: AgentState):
        """Determine if share-unlock analysis should continue."""
        messages = state["messages"]
        last_message = messages[-1]
        if last_message.tool_calls:
            return "tools_unlock"
        return "Msg Clear Unlock"
